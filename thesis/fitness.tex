...
access to the \texttt{rip} out of the box. Whenever a buffer is freed by a call to \texttt{free}, the
corresponding allocation is removed from the set of allocations. 

This set is just an auxiliary data structure that does not provide any useful information in terms of the
planned fitness score. Therefore, a second data structure is necessary for capturing the actual memory accesses
- an associative mapping from \texttt{rip}s to the maximum access distance from the center of a buffer
pertaining to an allocation before the \texttt{rip}. By maximizing the distance from the middle as a metric it
is possible to aim for accesses at both extremes of a buffer, thus giving preference to both overflows and
underflows at the same time. \improvement{Add illustration [...$|$.x...]}

Having established the desired data model and coming back to the problem of capturing buffer allocations as they
happen in a possibly multi-threaded application, a solution is to use {\small Pin's} support for
instrumentation point control and have two callbacks - one at the entry of the allocation function, and the
other at the exit. With this setup the \texttt{rip} and the \texttt{size} values are accessible to the first
callback and \texttt{rip} and \texttt{start} to the second. 
