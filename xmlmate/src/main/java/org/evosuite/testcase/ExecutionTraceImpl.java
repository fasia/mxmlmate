...
public class ExecutionTraceImpl implements ExecutionTrace, Cloneable {
	...
	public void exitMethod(String classname, String methodname) {
		if (traceCalls) {
			if (!stack.isEmpty() && !(stack.peek().methodName.equals(methodname))) {
                logger.debug("Expecting {}, got {}", stack.peek().methodName, methodname);
	...
	public synchronized void finishCalls() {
        logger.debug("At the end, we have {} calls left on stack", stack.size());
		while (!stack.isEmpty()) {
			finishedCalls.add(stack.pop());
		}
	}
	...
	public void linePassed(String className, String methodName, int line) {
		if (traceCalls) {
			if (stack.isEmpty()) {
                logger.info("Method stack is empty: {}.{} - l{}", className, methodName, line); // TODO switch back
				// logger.debug to
				// logger.warn
			} else {
				boolean empty = false;
				if (!stack.peek().methodName.equals(methodName)) {
					...
					if (stackHasMethod(methodName)) {
						do {
                            logger.debug("Popping method {} because we were looking for {}", stack.peek().methodName, methodName);
							finishedCalls.add(stack.pop());
						} while (!stack.isEmpty()
                        ...
                        logger.warn("Popping method {} because we were looking for {}", stack.peek().methodName, methodName);
                        logger.warn("Current stack: {}", stack);
						finishedCalls.add(stack.pop());
					}
					if (stack.isEmpty()) {
                        logger.warn("Method stack is empty: {}.{} - l{}", className, methodName, line); // TODO switch back
						empty = true;
					}
				}
				if (!empty)
