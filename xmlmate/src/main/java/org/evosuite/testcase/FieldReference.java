...
public class FieldReference extends VariableReferenceImpl {
		...
		try {
			...
			if (field.getField().getType().equals(int.class))
				...
				if (value != null
                    ...
                    logger.error("Not assignable: {} of class {} to field {} of variable {}", value, value.getClass(), field, source);
				}
            ...
            logger.error("Error while assigning field: {} with value {} on object {}: {}", getName(), value, sourceObject, e, e);
			throw e;
		} catch (IllegalAccessException e) {
            logger.error("Error while assigning field: {} of type: {} {}", field.getField().toString(), field.getField().getType().getCanonicalName(), e, e);
			throw new EvosuiteError(e);
		} catch (NullPointerException e) {
			throw new CodeUnderTestException(e);
		} catch (ExceptionInInitializerError e) {
	...
	public void setAdditionalVariableReference(VariableReference var) {
		if (source != null
            ...
            logger.info("Not assignable: {} and {}", field.getField().getDeclaringClass(), var);
			assert (false);
		}
		source = var;
	}
