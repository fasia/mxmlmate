...
public class TestChromosome extends ExecutableChromosome {
	...
	private boolean mutationInsert() {
		...
		while (Randomness.nextDouble() <= StrictMath.pow(ALPHA, count)
	...
	private boolean mutationConcolic() {
        ...
        logger.debug("Trying to negate branch {} - have {}/{} target branches", branch.getInstructionIndex(), targetBranches.size(), branches.size());
