	...
	private void readTestCases() throws IllegalStateException {
		...
		for (TestCase test : listener.getTestCases()) {
			...
			if (executionResult.noThrownExceptions()) {
                ...
                logger.info("Exception thrown in carved test: {}", executionResult.getExceptionThrownAtPosition(executionResult.getFirstPositionOfThrownException()));
		...
		if (junitTests.size() > 0) {
            ...
            logger.warn("It was not possible to carve any test case from: {}. Test execution results: {}", Arrays.toString(junitTestNames.toArray()), outcome);
		}
