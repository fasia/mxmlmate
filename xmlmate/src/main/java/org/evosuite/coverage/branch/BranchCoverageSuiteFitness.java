...
//		handleConstructorExceptions(results, callCount);
		...
		if (Properties.BRANCH_STATEMENT) {
            ...
            logger.info("Covered {} out of {} lines", covered_lines.size(), totalLines);
	...
	private void printStatusMessages(
		...
		if (coveredBranches > maxCoveredBranches) {
            ...
            logger.info("(Branches) Best individual covers {}/{} branches and {}/{} methods", coveredBranches, totalBranches * 2, coveredMethods, totalMethods);
            logger.info("Fitness: {}, size: {}, length: {}", fitness, suite.size(), suite.totalLengthOfTestCases());
		}
		if (coveredMethods > maxCoveredMethods) {
            logger.info("(Methods) Best individual covers {}/{} branches and {}/{} methods", coveredBranches, totalBranches * 2, coveredMethods, totalMethods);
			maxCoveredMethods = coveredMethods;
            logger.info("Fitness: {}, size: {}, length: {}", fitness, suite.size(), suite.totalLengthOfTestCases());

		}
		if (fitness < bestFitness) {
            logger.info("(Fitness) Best individual covers {}/{} branches and {}/{} methods", coveredBranches, totalBranches * 2, coveredMethods, totalMethods);
			bestFitness = fitness;
            logger.info("Fitness: {}, size: {}, length: {}", fitness, suite.size(), suite.totalLengthOfTestCases());
