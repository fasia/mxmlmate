		...
		for (MethodCall call : result.getTrace().getMethodCalls()) {
			...
			if (call.className.equals(lcsaj.getClassName())
				...
				for (int i = 0; i < call.branchTrace.size(); i++) {
                    ...
                    logger.debug("Current branch: {}: {}/{}, need {}", call.branchTrace.get(i), true_distance, false_distance, lcsaj.getBranchID(lcsaj_position));
				...
				if (Properties.STRATEGY != Strategy.EVOSUITE) {
					if (!found) {
                        logger.debug("Looking for approach to initial branch: {} with ID {}", lcsaj.getStartBranch(), lcsaj.getStartBranch().getActualBranchId());
