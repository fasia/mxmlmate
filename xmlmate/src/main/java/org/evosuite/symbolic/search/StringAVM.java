...
public final class StringAVM {
	...
	private boolean doCharacterAVM(int position) {
		...
		while (!done) {
			...
			double newDist = DistanceEstimator.getDistance(cnstr);
            log.debug("Probing increment {}: {}: {} replacement = {}", position, newString, newDist, (int) replacement);
			if (distImpr(newDist)) {
				checkpointVar(newDist);
                ...
                log.debug("Probing decrement {}: {}: {} replacement = {}", position, newString, newDist, (int) replacement);
				if (distImpr(newDist)) {
					checkpointVar(newDist);
					...
					strVar.setConcreteValue(newString);
					newDist = DistanceEstimator.getDistance(cnstr);
                    log.debug("Probing increment [32] {}: {}: {} replacement = {}", position, newString, newDist, (int) replacement);
					if (distImpr(newDist)) {
						checkpointVar(newDist);
	...
	private boolean iterateCharacterAVM(int position, int delta) {
        ...
        log.debug("Trying increment {} of {}", delta, oldString);
