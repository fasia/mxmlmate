...
public final class RealAVM {
	...
	private boolean doRealSearch(double delta,
		...
		while (true) {
			// Try increment
            log.debug("Trying to increment {} with: {}", realVar, delta);
			incrementVar(delta);
			double newDist = DistanceEstimator.getDistance(cnstr);
            log.debug("Old distance: {}, new distance: {}", this.checkpointedDistance, newDist);
			if (distImpr(newDist)) {
				improvement = true;
				...
				restoreVar();
				// Try decrement
                log.debug("Trying to decrement {} with: {}", realVar, delta);
				incrementVar(-delta);
	...
	private void restoreVar() {
        log.debug("restoring to: {} with dist: {}", realVar, checkpointedDistance);
	...
	private boolean afterCommaSearch(RealVariable realVar,
		...
		for (int precision = 1; precision <= maxPrecision; precision++) {
			...
			final double delta = StrictMath.pow(10.0, -precision);
	...
	private void iterateVar(double delta, double factor) {
        ...
        log.debug("[Loop] Trying increment {} of {}", delta, realVar.toString());

		incrementVar(delta);
		double newDist = DistanceEstimator.getDistance(cnstr);
        log.debug("[Loop] Old distance: {}, new distance: {}", this.checkpointedDistance, newDist);
		while (distImpr(newDist)) {
			...
			delta = factor * delta;
            log.debug("[Loop] Trying increment {} of {}", delta, realVar);
			incrementVar(delta);
			newDist = DistanceEstimator.getDistance(cnstr);
		}
