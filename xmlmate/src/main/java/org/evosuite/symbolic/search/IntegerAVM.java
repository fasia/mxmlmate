...
public final class IntegerAVM {
	...
	public boolean applyAVM() {
		...
		while (true) {
            ...
            log.debug("newDist: {} oldDist: {}", newDist, checkpointedDistance);
			if (distImpr(newDist)) {
				improvement = true;
	...
	private void restoreVar() {
        log.debug("restoring to: {} with dist: {}", intVar, checkpointedDistance);
	...
	private void iterateVar(long delta) {
        ...
        log.debug("Trying increment {} of {}", delta, intVar.toString());
		...
		while (distImpr(newDist)) {
			...
			delta = 2 * delta;
            log.debug("Trying increment {} of {}", delta, intVar);
			incrementVar(delta);
			newDist = DistanceEstimator.getDistance(cnstr);
            log.debug("newDist: {} oldDist: {}", newDist, checkpointedDistance);
		}
