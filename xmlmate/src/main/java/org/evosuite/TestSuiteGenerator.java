...
public class TestSuiteGenerator {
	...
	public TestGenerationResult generateTestSuite() {
		...
		try {
            ...
            LoggingUtils.getEvoLogger().error("* Error while initializing target class: {}", e.getMessage() != null ? e.getMessage()
                    : e.toString());
            logger.error("Problem for {}. Full stack:", Properties.TARGET_CLASS, e);
	...
	private List<TestCase> generateTests() {
        ...
        LoggingUtils.getEvoLogger().info("* Time spent executing tests: {}ms", TestCaseExecutor.timeExecuted);

		if (Properties.CRITERION == Criterion.DEFUSE) {
			if (Properties.ENABLE_ALTERNATIVE_FITNESS_CALCULATION)
                LoggingUtils.getEvoLogger().info("* Time spent calculating alternative fitness: {}ms", DefUseFitnessCalculator.alternativeTime);
            LoggingUtils.getEvoLogger().info("* Time spent calculating single fitnesses: {}ms", DefUseCoverageTestFitness.singleFitnessTime);
		}
	...
	public TestSuiteChromosome generateWholeSuite() {
		...
		if (ga == null || ga.getAge() == 0) {
            ...
            LoggingUtils.getEvoLogger().info("* Resuming search algorithm at generation {} for whole suite generation", ga.getAge());
		}
...
//		ga.addListener(progressMonitor); // FIXME progressMonitor may cause
		...
		if (Properties.SHOW_PROGRESS)
			LoggingUtils.getEvoLogger().info("");
        LoggingUtils.getEvoLogger().info("* Search finished after {}s and {} generations, {} statements, best individual has fitness {}", end_time - start_time, ga.getAge(), MaxStatementsStoppingCondition.getNumExecutedStatements(), best.getFitness());
        ...
        LoggingUtils.getEvoLogger().info("* Generated {} tests with total length {}", best.size(), best.totalLengthOfTestCases());
        ...
        LoggingUtils.getEvoLogger().info("* Resulting test suite's coverage: {}", NumberFormat.getPercentInstance().format(best.getCoverage()));
	...
	public static TestSuiteFitnessFunction getFitnessFunction(Criterion criterion) {
		...
		default:
            logger.warn("No TestSuiteFitnessFunction defined for {} using default one (BranchCoverageSuiteFitness)", Properties.CRITERION);
			return new BranchCoverageSuiteFitness();
		}
	}
	...
	public TestSuiteChromosome generateIndividualTests() {
		...
		while (current_budget < total_budget && covered_goals < total_goals
			...
			for (TestFitnessFunction fitnessFunction : goals) {
				...
				if (ga.getBestIndividual().getFitness() == 0.0) {
                    ...
                    logger.info("Found no solution for {} at {}", fitnessFunction, MaxStatementsStoppingCondition.getNumExecutedStatements());
				}
		...
		statistics.searchFinished(suiteGA);
		long end_time = System.currentTimeMillis() / 1000;
        LoggingUtils.getEvoLogger().info("* Search finished after {}s, {} statements, best individual has fitness {}", end_time - start_time, current_budget, suite.getFitness());
        ...
        logger.info("Resulting test suite: {} tests, length {}", suite.size(), suite.totalLengthOfTestCases());
        ...
        LoggingUtils.getEvoLogger().info("* Resulting test suite: {} tests, length {}", suite.size(), suite.totalLengthOfTestCases());
