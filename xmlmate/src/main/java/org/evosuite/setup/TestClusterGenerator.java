...
public class TestClusterGenerator {
	...
	private boolean addCastClassDependencyIfAccessible(String className,
		...
		try {
            ...
            logger.error("Problem for {}. Class not found", Properties.TARGET_CLASS, e);
			blackList.add(className);
			return false;
		}
	}
	...
	private List<GenericClass> getAssignableTypes(java.lang.reflect.Type type) {
		...
		for (GenericClass clazz : genericCastClasses) {
			if (clazz.isAssignableTo(type)) {
                logger.debug("{} is assignable to {}", clazz, type);
				types.add(clazz);
			}
		}
		return types;
	...
	private void initializeTargetMethods() throws RuntimeException,
		...
		while (!innerClasses.isEmpty()) {
			...
			try {
                ...
                logger.error("Problem for {}. Error loading inner class: {}, {},{}: {}", Properties.TARGET_CLASS, icn.innerName, icn.name, icn.outerName, t);
			}
		}
		...
		for (Class<?> clazz : targetClasses) {
			...
			for (Method method : getMethods(clazz)) {
				...
				if (canUse(method, clazz)) {
                    logger.debug("Adding method {}.{}{}", clazz.getName(), method.getName(), Type.getMethodDescriptor(method));
	...
	public static Set<Field> getAccessibleFields(Class<?> clazz) {
		...
		try {
            ...
            logger.info("Error while accessing fields of class {} - check allowed permissions: {}", clazz.getName(), t);
		}
		return fields;
	}
		...
		try {
			...
			for (Method method : getMethods(clazz.getRawClass())) {
				...
				if (canUse(method, clazz.getRawClass())
                    ...
                    logger.debug("Adding method {}.{}{}", clazz.getClassName(), method.getName(), Type.getMethodDescriptor(method));
			 ...
			 */
            logger.error("Problem for {}. Failed to add dependencies for class {}: {}\n{}", Properties.TARGET_CLASS, clazz.getClassName(), t, Arrays.asList(t.getStackTrace()));
				...
				for (String subClass : subClasses) {
					if (classDistance.get(subClass) == distance) {
						try {
                            ...
                            logger.error("Problem for {}. Class not found: {}", Properties.TARGET_CLASS, subClass, e);
