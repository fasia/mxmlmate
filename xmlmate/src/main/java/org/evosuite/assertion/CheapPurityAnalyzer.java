...
public class CheapPurityAnalyzer {
	...
	private void addCacheValue(MethodEntry entry, boolean new_value) {
		if (isCached(entry)) {
			...
			if (old_value != new_value) {
                ...
                logger.warn("The method {} had a different value in the purity cache (old_value={},new_value={})", fullyQuantifiedMethodName, old_value, new_value);
			}
		}
	...
	private boolean checkAnyOverridingMethodImpure(MethodEntry entry,
		...
		if (!inheritanceTree.hasClass(entry.className)) {
            logger.warn("{} was not found in the inheritance tree. Using DEFAULT value for cheap-purity analysis", entry.className);
			return DEFAULT_PURITY_VALUE;
		}
