...
public class SimpleMutationAssertionGenerator extends
	...
	public void addAssertions(TestCase test, Set<Integer> killed,
		...
		for (Mutation m : executedMutants) {
			...
			*/
            ...
            logger.debug("Running test {} on mutation {}", test.hashCode(), m.getMutationName());
        ...
        logger.info("Got {} assertions", assertions.size());
	...
	private void minimize(TestCase test, List<Mutation> mutants,
        ...
        logger.debug("Minimized assertions from {} to {}", assertions.size(), result.size());
