...
public class DescriptorMapping {
	...
	private String transformMethodName(String className, String methodName, String desc,
		...
		while (!parents.isEmpty()) {
			...
			visited.add(name);
            logger.info("Visiting class {} while looking for name clashes of {}.{}{}", name, className, methodName, transformedDesc);
			ClassReader reader;
			try {
				reader = new ClassReader(name);
				ClassNode parent = new ClassNode();
			...
			if (!descriptorMapping.containsKey(old)) {
				if (isOutsideMethod(className, methodName, desc)) {
					...
					if (newName != null) {
						...
						//                newName);
                        logger.info("Keeping transformation from {} to {} with new name {}", old, descriptorMapping.get(old), newName);
