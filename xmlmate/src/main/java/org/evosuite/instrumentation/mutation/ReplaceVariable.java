...
public class ReplaceVariable implements MutationOperator {
	...
	private Map<String, InsnList> getReplacements(MethodNode mn, String className,
		...
		if (node instanceof VarInsnNode) {
			...
			try {
                ...
                logger.debug("Looking for replacements for {} of type {} at index {}", origVar.name, origVar.desc, origVar.index);
			...
			if (field.owner.replace("/", ".").equals(className)) {
                logger.info("Looking for replacements for static field {} of type {}", field.name, field.desc);
	...
	private LocalVariableNode getLocal(MethodNode mn, AbstractInsnNode node, int index)
		...
		for (Object v : mn.localVariables) {
            ...
            logger.info("Checking {} in scope {} - {}", localVar.index, startId, endId);
	...
	private Map<String, InsnList> getLocalReplacements(MethodNode mn, String desc,
		...
		int currentId = mn.instructions.indexOf(node);
        logger.info("Looking for replacements at position {} of variable {} of type {}", currentId, otherNum, desc);
		...
		for (Object v : mn.localVariables) {
			...
			int startId = mn.instructions.indexOf(localVar.start);
			int endId = mn.instructions.indexOf(localVar.end);
            logger.info("Checking local variable {} of type {} at index {}", localVar.name, localVar.desc, localVar.index);
			...
			if (localVar.desc.equals(desc) && localVar.index != otherNum
                ...
                logger.info("Adding local variable {} of type {} at index {},  {}-{}, {}", localVar.name, localVar.desc, localVar.index, startId, endId, currentId);
	...
	private Map<String, InsnList> getLocalReplacementsInc(MethodNode mn, String desc,
		...
		for (Object v : mn.localVariables) {
			...
			if (localVar.desc.equals(desc) && localVar.index != otherNum
                ...
                logger.info("Adding local variable {} of type {} at index {}", localVar.name, localVar.desc, localVar.index);
				InsnList list = new InsnList();
