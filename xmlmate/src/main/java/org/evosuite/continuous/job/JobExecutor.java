...
public class JobExecutor {
	...
	public synchronized void executeJobs(final List<JobDefinition> jobs, final int cores) throws IllegalStateException{
        ...
        logger.info("Going to execute {} jobs", jobs.size());
		...
		Thread mainThread = new Thread(){
			...
			public void run(){
				...
				try{
                    LoggingUtils.getEvoLogger().info("Going to execute {} jobs", jobs.size());
					longestJob = execute(jobs);
				} catch(Exception e){
                    logger.error("Error while trying to execute the {} jobs: {}", jobs.size(), e.getMessage(), e);
				}
				finally {
					/*
