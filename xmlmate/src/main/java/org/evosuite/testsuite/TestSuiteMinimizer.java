	...
	private void filterJUnitCoveredGoals(List<TestFitnessFunction> goals) {
		if(!Properties.JUNIT_EXTEND.isEmpty()) {
			...
			try {
                ...
                logger.warn("Removing {} goals already covered by JUnit (total: {})", coveredGoals.size(), goals.size());
                logger.warn("Removing {} goals already covered by JUnit (total: {})", coveredGoals, goals);
	...
	public void minimizeTests(TestSuiteChromosome suite) {
		...
		for (TestFitnessFunction goal : goals) {
				...
				if (goal.isCovered(test)) {
					if (Properties.ASSERTION_STRATEGY == AssertionStrategy.STRUCTURED) {
						...
						if (structuredTest.getTargetMethods().contains(goal.getTargetMethod())) {
                            logger.info("Covered by minimized test targeting {}: {} ", structuredTest.getTargetMethods(), goal);
							covered.add(goal);
			...
			if (covered.contains(goal)) {
                ...
                logger.info("Now the suite covers {}/{} goals", covered.size(), goals.size());
				continue;
			}
			...
			if (!coveredTests.isEmpty()) {
                ...
                logger.info("After new test the suite covers {}/{} goals", covered.size(), goals.size());
        ...
        logger.info("Minimized suite covers {}/{} goals", covered.size(), goals.size());
		suite.tests.clear();
		...
		while (changed && !isTimeoutReached()) {
			...
			for (TestChromosome testChromosome : suite.tests) {
				...
				for (int i = testChromosome.size() - 1; i >= 0; i--) {
                    ...
                    logger.debug("Deleting statement {} from test", testChromosome.getTestCase().getStatement(i).getCode());
					...
					if (Double.compare(modifiedVerFitness, fitness) <= 0) {
                        ...
                        logger.debug("Can't remove statement {}", orgiginalTestChromosome.getTestCase().getStatement(i).getCode());
                        logger.debug("Restoring fitness from {} to {}", modifiedVerFitness, fitness);
						testChromosome.setTestCase(orgiginalTestChromosome.getTestCase());
