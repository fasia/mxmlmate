...
public class FloatLocalSearch<T extends Number> extends StatementLocalSearch {
	...
	private static final Logger logger = LoggerFactory.getLogger(FloatLocalSearch.class);
	...
	public boolean doSearch(TestChromosome test, int statement,
		...
		for (int precision = 1; precision <= maxPrecision; precision++) {
			...
			change = doSearch(test, statement, objective, StrictMath.pow(10.0, -precision), 2, p);
			if(change < 0)
				improved = true;
		}
	...
	private boolean iterate(double delta, double factor, LocalSearchObjective<TestChromosome> objective,
		...
		ExecutionResult oldResult = test.getLastExecutionResult();
        logger.info("Trying increment {} of {}", delta, p.getCode());
