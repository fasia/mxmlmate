	...
	public boolean doSearch(TestChromosome individual,
		...
		for (int i = lastPosition; i >= 0; i--) {
			...
			if (!test.hasReferences(test.getStatement(i).getReturnValue())
                ...
                logger.info("Return value of statement {} is not referenced and not SUT, not doing local search", i);
				continue;
			}
			...
			if(Properties.LOCAL_SEARCH_DSE == DSEType.TEST)
				...
				if (search != null) {
                    logger.info("Applying local search of type {} to statement {} / {}", search.getClass(), test.getStatement(i), individual.getTestCase().getStatement(i));
					if(search.doSearch(individual, i, (LocalSearchObjective<TestChromosome>) objective))
						result = true;
					// i = s.getPosition();
                    logger.info("Old position was: {}, adjusting to: {}", i, i + search.getPositionDelta());
