		...
		if (otherRawClass.isAssignableFrom(rawClass)) {
			...
			try {
				...
				if (equals(instantiation)) {
                    logger.debug("Instantiation is equal to original, so I think we can't assign: {}", instantiation);
		...
		while(changed) {
			...
			for (TypeVariable<?> var : ownerVariableMap.keySet()) {
				...
				if(ownerVariableMap.get(var) instanceof TypeVariable<?>) {
                    logger.debug("Is set to type var: {} of {}", ownerVariableMap.get(var), ((TypeVariable<?>) ownerVariableMap.get(var)).getGenericDeclaration());
