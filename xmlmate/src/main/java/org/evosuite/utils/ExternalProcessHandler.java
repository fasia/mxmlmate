			...
			public void run() {
				...
				while (read && !isInterrupted()) {
					...
					try {
						 ...
						 */
                        logger.error("Class {}. Error in reading message. Likely the client has crashed. Error message: {}", Properties.TARGET_CLASS, e.getMessage());
					...
					if (message.equals(Messages.FINISHED_COMPUTATION)) {
						...
						killProcess();
                        logger.error("Class {}. Error, received invalid message: ", Properties.TARGET_CLASS, message);
						return;
					}
				}
			}
		...
		try {
			...
			for(ClientNodeRemote client : clients){
				...
				if(!finished){
					 ...
					 */
                    logger.error("Class {}. Clients have not finished yet, although timeout.\n{}", Properties.TARGET_CLASS, MasterServices.getInstance().getMasterNode().getSummaryOfClientStatuses());
				}				
			}
		} catch (InterruptedException e) {		
		} catch(RemoteException e){
            logger.error("Class {}. Lost connection with clients.\n{}", Properties.TARGET_CLASS, MasterServices.getInstance().getMasterNode().getSummaryOfClientStatuses(), e);
		}
