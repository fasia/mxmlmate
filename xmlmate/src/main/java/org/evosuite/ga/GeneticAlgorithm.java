...
public abstract class GeneticAlgorithm<T extends Chromosome> implements SearchAlgorithm, Serializable {
	...
	/** Current population */
	protected List<T> population = new ArrayList<T>(Properties.POPULATION);
...
*/
	...
	protected void generateRandomPopulation(int population_size) {
        ...
        logger.debug("Created {} individuals", population.size());
	}
	...
	protected void calculateFitnessAndSortPopulation() {
        logger.debug("Calculating fitness for {} individuals", population.size());
	...
	protected List<T> elitism() {
		...
		for (int i = 0; i < Properties.ELITE; i++) {
            logger.trace("Copying individual {} with fitness {}", i, population.get(i).getFitness());
	...
	public boolean isFinished() {
		for (StoppingCondition c : stoppingConditions) {
			if (c.isFinished()) {
                logger.info("Stopping search due to {}", c);
                return true;
            }
		}
		return false;
