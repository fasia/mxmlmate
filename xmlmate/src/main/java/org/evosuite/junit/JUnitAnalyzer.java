...
public class JUnitAnalyzer {
	...
	public static int handleTestsThatAreUnstable(List<TestCase> tests) {
		...
		try {
            ...
            logger.error("{} test cases failed", result.getFailureCount());
		...
		try {
			...
			if (!result.wasSuccessful()) {
				...
				for (JUnitFailure failure : result.getFailures()) {
                    logger.error("Failure {}: {}\n{}", failure.getExceptionClassName(), failure.getMessage(), failure.getTrace());
				}
				return false;
			} else {
				/*
	...
	private static Class<?>[] getClassesFromFiles(Collection<File> files) {
		...
		for (File file : files) {
			...
			try {
                ...
                logger.error("Failed to load test case {} from file {} , error {}", className, file.getAbsolutePath(), e, e);
				return null;
			}
