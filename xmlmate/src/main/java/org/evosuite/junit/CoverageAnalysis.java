...
public class CoverageAnalysis {
	...
	public static void analyzeCoverage() {
        ...
        LoggingUtils.getEvoLogger().info("* Found {} unit test classes", junitTests.size());
	...
	private static double getCoverage(List<JUnitResult> results) {
        ...
        LoggingUtils.getEvoLogger().info("* Executed {} tests", results.size());
	...
	private static List<Class<?>> getClassesFromClasspath() {
		...
		for(String prefix : Properties.JUNIT_PREFIX.split(":")) {
            ...
            LoggingUtils.getEvoLogger().info("* Found {} classes with prefix {}", suts.size(), prefix);
			if (!suts.isEmpty()) {
				for (String sut : suts) {
					try {
	...
	private static void printReport(List<JUnitResult> results, List<Class<?>> classes, long startTime) {
        ...
        LoggingUtils.getEvoLogger().info("* Covered {}/{} coverage goals: {}", coveredGoals.cardinality(), goals.size(), NumberFormat.getPercentInstance().format((double) coveredGoals.cardinality()
                / (double) goals.size()));
