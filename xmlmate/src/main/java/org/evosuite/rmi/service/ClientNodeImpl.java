...
public class ClientNodeImpl implements ClientNodeLocal, ClientNodeRemote {
		...
		searchExecutor.submit(new Runnable() {
			...
			public void run() {
				...
				try {
                    ...
                    logger.error("Error when generating tests for: {} with seed {}. Configuration id : {}", Properties.TARGET_CLASS, Randomness.getSeed(), Properties.CONFIGURATION_ID, t);
	...
	public void changeState(ClientState state, ClientStateInformation information) {
		if (this.state != state){
            logger.info("Client changing state from {} to {}", this.state, state);
		}
		...
		searchExecutor.submit(new Runnable() {
			...
			public void run() {
				...
				try {
                    ...
                    logger.error("Error when analysing coverage for: {} with seed {}. Configuration id : {}", Properties.TARGET_CLASS, Randomness.getSeed(), Properties.CONFIGURATION_ID, t);
				}
