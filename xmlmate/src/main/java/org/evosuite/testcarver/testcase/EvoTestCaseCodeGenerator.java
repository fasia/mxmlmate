...
public final class EvoTestCaseCodeGenerator implements ICodeGenerator<TestCase> {
	...
	public void createUnobservedInitStmt(CaptureLog log, int logRecNo) {
		...
		try {
			...
			final MethodStatement m = new MethodStatement(testCase, new GenericMethod(EvoSuiteXStream.class.getMethod("fromString", new Class<?>[] {String.class}), EvoSuiteXStream.class), null, Collections.singletonList(stringRepRef));
...
//				        fieldType);
                ...
                LoggingUtils.getEvoLogger().debug("Error while trying to get field {} of class {}: {}", fieldName, getClassForName(typeName), e);
	...
	private Method getDeclaredMethod(final Class<?> clazz, final String methodName,
        ...
        logger.info("Trying to get method {} from class {} with parameters {}", methodName, clazz, Arrays.asList(paramTypes));
		if (clazz == null || Object.class.equals(clazz)) {
