 ...
 */
public class BothCoveragesMonitor implements SearchListener {
    private static final Logger logger = LoggerFactory.getLogger(BothCoveragesMonitor.class);
    private final BufferedWriter writer;
    private int lastUpdate = 0;
    private double lastFitness = Double.MAX_VALUE;

    public BothCoveragesMonitor() {
        File output = new File(XMLProperties.OUTPUT_PATH, XMLProperties.RUN_NAME + " Combined.log");
    ...
    public void iteration(GeneticAlgorithm<?> algorithm) {
        ...
        if (newFitness < lastFitness) {
            ...
            try {
                ...
                double cov = xsuite.getCoverage();
                writer.write(String.format(Locale.US, "%04d,%.4f,%.4f,%.4f,%.4f,%.4f,", age, el, at, re, cov, newFitness));
                writer.newLine();
